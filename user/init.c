...
{
	cwrite(
	"kappara -- a small SVR4-flavored Unix-like OS for AArch64.\r\n"
	"\r\n"
	"Built for fun and for the education of future software\r\n"
	"engineers: one readable code base pulling the best ideas from\r\n"
	"the UNIX family tree -- SVR4/Solaris STREAMS and clone devices,\r\n"
	"BSD reliable signals and the ARP hold queue, classic vnodes,\r\n"
	"POSIX numbering -- built from scratch, no borrowed code.\r\n"
	"Develops on QEMU virt; runs on real AWS Graviton (UEFI + ENA\r\n"
	"+ NVMe).  Docs live in docs/ -- start with ARCHITECTURE.md.\r\n"
	"\r\n");
	/* Split into multiple writes: a single cwrite of the whole text
	 * exceeds allocb's 2 KB kmalloc cap and prints nothing. */
	cwrite(
	"shell builtins:\r\n"
	"  help                   this text\r\n"
	"  pid                    sys_getpid\r\n"
	"  pwd / cd [path]        show / change current directory\r\n"
	"  ls | ll [path]         list a directory (ll = type + size)\r\n"
	"  cat <path>             dump file to console\r\n"
	"  echo <path> <text>     write text to file (overwrite)\r\n"
	"  append <path> <text>   append text + newline to file\r\n"
	"  touch / mkdir <path>   create empty file / directory (kfs)\r\n"
	"  rm / rmdir <path>      remove file / empty directory\r\n"
	"  ked | vi <path>        line editor / vi-lite full-screen editor\r\n"
	"  kc                     two-panel file manager (Norton style)\r\n"
	"  more <path>            page through a file\r\n"
	"  open <path>            open and remember as $fd\r\n"
	"  close / read / write   drive $fd by hand\r\n"
	"  push <mod> / pop       STREAMS I_PUSH / I_POP on $fd\r\n"
	"  spawn [arg]            sys_spawn a worker thread\r\n"
	"  kill <tid> [sig]       POSIX signals (default SIGTERM=15)\r\n"
	"  exec <path>            load & run an ELF\r\n"
	"  ftrace [on|off|reset|dump]  per-CPU function tracer\r\n"
	"  halt                   ask QEMU to exit (semihosting)\r\n"
	"\r\n");
	cwrite(
	"networking (STREAMS mux; drivers are DLPI datalinks):\r\n"
	"  ifconfig [if [ip netmask m gw g]]  show / plumb interfaces\r\n"
	"  ping [ip] [count]      ICMP echo via /dev/icmp\r\n"
	"  host <name>            DNS A lookup (server from DHCP opt 6)\r\n"
	"  netstat                socket / connection summary\r\n"
	"  tcpconnect <ip> <port> raw TCP client via /dev/tcp\r\n"
	"  dhcpagent [if]         userland DHCP over /dev/eth0 (runs at boot)\r\n"
	"  daemons: telnetd :23 -> shell on tty4, ftpd :21 -> /home\r\n"
	"\r\n"
	"introspection under /proc (cat any of them):\r\n"
	"  ps meminfo slabinfo streams arp netif tcp cpuload mounts\r\n"
	"  ftrace acpi pci efi nvme slip\r\n"
	"\r\n"
	"devices: /dev/tcp udp icmp are clone devices (fresh stream per\r\n"
	"open); /dev/eth0 is the raw datalink (mini-DLPI); /dev/nvme0n1\r\n"
	"backs the persistent /home; tty0-4, console, klog, ksyms, loop.\r\n"
	"\r\n"
	"  <name>                 anything else runs /usr/bin/<name>\r\n"
	"                         (ps uptime wc grep head tail cp mv\r\n"
	"                          nm ldd objdump mount test <x>|all)\r\n");
}
