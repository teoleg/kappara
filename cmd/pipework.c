...
static void pipe_reader(long packed) {
    ...
    sys_close(discard);
    for (;;) {
        char buf[80]; long n = sys_read(keep, buf, sizeof(buf) - 1);
        if (n <= 0) break;
        buf[n] = '\0';
    ...
    sys_close(keep); sys_exit();
}
void _start(void) {
    int fds[2];
    if (sys_pipe(fds) < 0) { err("pipework: sys_pipe failed\r\n"); sys_exit(); }
    ...
    out("pipework: writer tid="); out_long(wtid); out(", reader tid="); out_long(rtid); out("\r\n");
